...
const WS_URL = `${WS_PROTOCOL}//${window.location.host}/ws/transactions`;
...
function appendRow(txn, isLive = true) {
  ...
  const senderCountry = String(txn.sender_country || "").toUpperCase().slice(0, 2);
  const receiverCountry = String(txn.receiver_country || "").toUpperCase().slice(0, 2);
  ...
  tr.innerHTML = `
    ...
    <td>${senderCountry} ➔ ${receiverCountry}</td>
